...
class GoodsFirebase {
  ...
  /* 데이터 추가 */
  static Future add(BuildContext context, Goods goods,
      List<File?> thumbnailImageList, List<File?> detailImageList) async {
    ...
    for (int i = 0; i < thumbnailImageList.length; i++) {
      await _uploadImageFile(
              docRef.id, 'thumbnailImage$i', thumbnailImageList[i]!)
          .then((value) {
        if (value != null) {
    ...
    for (int i = 0; i < detailImageList.length; i++) {
      await _uploadImageFile(docRef.id, 'detailImage$i', detailImageList[i]!)
          .then((value) {
        if (value != null) {
  ...
  static Future<Goods> getDataEqualUID(
      BuildContext context, String goodsId) async {
    List<Goods> goodsList = [];
    bool isNetwork = await Provider.of<NetworkProvider>(context, listen: false)
        .checkNetwork();
    try {
      // 인터넷 상태 확인
      if (!isNetwork) return Goods(status: true);
  ...
  static Future changeStatus(
      BuildContext context, String goodsId, bool status) async {
  ...
  static Future delete(BuildContext context, Goods goods) async {
    ...
    try {
      ...
      for (int i = 0; i < goods.thumbnailImages!.length; i++) {
        final ref =
            storageRef.child("goods/${goods.goodsId}/thumbnailImage$i.jpg");
        await ref.delete();
      }
      //상세 이미지 삭제
      for (int i = 0; i < goods.detailImages!.length; i++) {
        final ref =
            storageRef.child("goods/${goods.goodsId}/detailImage$i.jpg");
        await ref.delete();
      }
  ...
  static Future _uploadImageFile(
      String goodsId, String fileType, File file) async {
    try {
      final storageRef = storage.ref();
      final mountainsRef =
